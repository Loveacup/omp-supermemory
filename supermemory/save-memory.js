...
async function main() {
  ...
  const scopeFlag = process.argv.find(a => a.startsWith('--scope='))?.split('=')[1];
  const content = args.join(" ");
  if (!content.trim()) {
    console.log('No content provided. Usage: node save-memory.js "<content>" [--type=<type>] [--scope=<scope>]');
    console.log('Types: project-config, architecture, error-solution, preference, learned-pattern, conversation');
    console.log('Scopes: user, project (default)');
    process.exit(0);
  }
  const client = new SupermemoryClient();
  const tag = scopeFlag === 'user' ? getUserTag() : getProjectTag(process.cwd());
  try {
    const metadata = {
      type: typeFlag || "project-knowledge",
      source: "omp-windows",
      sm_source: "omp-windows",
      timestamp: (/* @__PURE__ */ new Date()).toISOString()
    };
    const result = await client.addMemory(content, tag, metadata);
    if (result.success) {
      console.log(`Memory saved (id: ${result.id}) to '${tag}'`);
    } else {
